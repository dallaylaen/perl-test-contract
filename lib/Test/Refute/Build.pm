...
Test::Refute::Build - tool for extending Test::Refute suite

=head1 DESCRIPTION

Unfortunately, extending L<Test::Refute> is not completely straightforward.
...
the most up-to-date L<Test::Refute::Contract> instance;

=item * add a method with the same name to L<Test::Refute::Contract>
...
Create a function in calling package and a method in L<Test::Refute::Contract>.
As a side effect, Test::Refute's internals are added to the caller's
...
See L<Test::Refute::Exception> for example (custom prototyped functions).
...
C<$contract> must be a L<Test::Refute::Contract> descendant.
